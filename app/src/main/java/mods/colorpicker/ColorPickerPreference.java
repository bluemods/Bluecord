...
public class ColorPickerPreference extends Preference {
	...
	private void initWith(Context context, AttributeSet attrs) {
		final TypedArray typedArray = context.obtainStyledAttributes(attrs, ColorPickerConstants.getDeclaredPickerAttributes());
		...
		setWidgetLayoutResource(ColorPickerConstants.LAYOUT_COLOR_WIDGET);
	}
	...
	protected void onBindView(@NonNull View view) {
		...
		colorIndicator = (ImageView) view.findViewById(android.R.id.icon1);
	...
	protected void onSetInitialValue(boolean restoreValue, Object defaultValue) {
		setValue(restoreValue ? getPersistedInt(0) : safeCast(defaultValue));
	}

	private static int safeCast(Object val) {
		try {
			if (val instanceof Integer) return (Integer) val;
			return Color.parseColor(val.toString());
		} catch (Exception e) {
			e.printStackTrace();
			return Color.WHITE;
		}
